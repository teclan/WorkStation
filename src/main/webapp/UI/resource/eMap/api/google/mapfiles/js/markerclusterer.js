...
function MarkerClusterer(map, opt_markers, opt_options) {
  ...
  this._callback = options['ClickClusterer'];
  ...
  //单击div __LY,20141217
  this.ClickDiv_ = null;
  if(options['ClickClusterer'] != null){
	  this.ClickDiv_ = function(data,param) {
        that.callback(data,param);
	  }
  }
  
  //单击marker __LY,20141217
  this.ClickMarker_ = null;
  if(options['ClickClusterer'] != null){
	  this.ClickMarker_ = function(data,param){
        that.callback([data],param);
	  }
  }
  
  //设置显示面板的最大地图级别
  var mapTypesId = that.map.getMapTypeId();
  var MaxZoom = 17;//mtd.map.mapTypes[mapTypesId].maxZoom;
  this.ClustererMaxZoom_ = options["ClustererMaxZoom"] || MaxZoom;
...
MarkerClusterer.prototype.callback = function(data,param){
  var _self = this;
  if(typeof _self._callback === 'function'){
    var rMarkers = _self._rMarkers;
    var resources = [];
    for(var i=0,marker;marker=data[i];i++){
        resources.push(rMarkers[marker['id']]);
    }
    var param = {
      type:'cluster_click',
      original:{
        lng:param['latLng'] ? param['latLng']['A']:data[0]['position']['A'],
        lat:param['latLng'] ? param['latLng']['k']:data[0]['position']['k']
      },
      data:resources
    }
    _self._callback(param);
  }
}
/**
...
MarkerClusterer.prototype.pushMarkerTo_ = function(marker) {
  ...
  if(!marker["isBindClick"]){
      google.maps.event.addListener(marker,'click',function(e){
        var fun = that.ClickMarker_;
        if(fun != null){
          fun(this,e);
        }
      });
    marker["isBindClick"] = true;
  }


  this.markers_.push(marker);
};
MarkerClusterer.prototype.setMarkers = function(markers){
    this.markers_ = markers&&markers.length>0 ? markers : [];
}
...
MarkerClusterer.prototype.setRMarkers = function(rMarkers) {
    this._rMarkers = rMarkers;
};
...
MarkerClusterer.prototype.createClusters_ = function() {
  ...
  for (var i = 0, marker; marker = markers[i]; i++) {
	  if(marker.isSetMap == false)continue;
    var added = false;
...
ClusterIcon.prototype.onAdd = function() {
  ...
  google.maps.event.addDomListener(this.div_, 'click', function(event) {
	  ...
	  //1201  --LY
	  //that.cluster_.markerClusterer_
	  
	  var zoom = that.map.getZoom();
	  var maxZoom = that.cluster_.markerClusterer_.ClustererMaxZoom_;
	  if(zoom >= maxZoom)
	  {
		  var aClustererArr = that.cluster_.markers_;
		  var fun = that.cluster_.markerClusterer_.ClickDiv_;
		  if(fun != null){
			  fun(aClustererArr,event);
		  }
		  that.triggerClusterClick();
